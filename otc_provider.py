...
INTÉGRATION OTC_PROVIDER pour les données crypto
"""
...
# IMPORT DU FOURNISSEUR OTC
from otc_provider import OTCDataProvider
print("✅ OTCDataProvider importé avec succès")
...
CRYPTO_PAIRS = [
    'BTC/USD',    # Bitcoin
    'ETH/USD',    # Ethereum
    'TRX/USD',    # Tron (TRX est souvent disponible sur OTC)
    'LTC/USD',    # Litecoin
]
...
# Initialisation OTC avec le nouveau fournisseur
otc_provider = OTCDataProvider()
print(f"✅ OTC Provider initialisé - Paires disponibles: {otc_provider.get_available_pairs()}")
...
def fetch_ohlc_with_limits(pair, interval, outputsize=300, signal_id=None):
    """
    Récupération données Forex avec gestion des limites API
...
def fetch_ohlc_otc(pair, interval, outputsize=300, signal_id=None):
    """
    Récupération données Crypto via OTC Provider
    """
    try:
        print(f"[OTC] 🔄 Récupération données pour {pair} (interval: {interval}, limit: {outputsize})")
        
        # Appeler le fournisseur OTC
        df = otc_provider.get_otc_data(pair, interval, outputsize)
        
        if df is None:
            raise RuntimeError(f"OTC Provider n'a pas retourné de données pour {pair}")
        
        if len(df) < ROTATION_CONFIG['min_data_points']:
            print(f"[OTC] ⚠️ Données insuffisantes: {len(df)} bougies (minimum: {ROTATION_CONFIG['min_data_points']})")
            raise RuntimeError(f"Données OTC insuffisantes: {len(df)} bougies")
        
        print(f"[OTC] ✅ Données OTC chargées: {len(df)} bougies pour {pair}")
        return df
        
    except Exception as e:
        error_msg = f"Erreur OTC Provider pour {pair}: {e}"
        add_error_log(error_msg)
        print(f"[OTC] ❌ {error_msg}")
        raise RuntimeError(f"Erreur OTC: {e}")
...
def get_cached_ohlc(pair, interval, outputsize=300, signal_id=None):
    ...
    if cache_key in ohlc_cache:
        ...
        if (current_time - cached_time).total_seconds() < 30:
            print(f"[CACHE] 📦 Utilisation données en cache pour {current_pair}")
            return cached_data
    
    try:
        # CHOIX DE LA SOURCE EN FONCTION DU MODE
        is_weekend = otc_provider.is_weekend()
        
        if is_weekend and current_pair in otc_provider.get_available_pairs():
            # MODE WEEK-END: Utiliser OTC Provider pour les crypto
            print(f"[OTC MODE] 🌙 Récupération OTC pour {current_pair}")
            df = fetch_ohlc_otc(current_pair, interval, outputsize, signal_id)
        else:
            # MODE FOREX: Utiliser TwelveData
            print(f"[FOREX MODE] 📊 Récupération TwelveData pour {current_pair}")
        ...
        # Fallback: si OTC échoue en week-end, essayer TwelveData
        if is_weekend and current_pair in otc_provider.get_available_pairs():
            print(f"[FALLBACK] 🔄 OTC échoué, tentative avec TwelveData pour {current_pair}")
            try:
                df = fetch_ohlc_with_limits(current_pair, interval, outputsize, signal_id)
                ohlc_cache[cache_key] = (df, current_time)
                return df
            except Exception as fallback_error:
                print(f"[FALLBACK] ❌ TwelveData a aussi échoué: {fallback_error}")
...
async def analyze_multiple_markets_iterative(user_id, session_count, signal_id=None):
    ...
    Rotation Crypto optimisée pour week-end avec OTC Provider
    ...
    is_weekend = otc_provider.is_weekend()
    if is_weekend:
        print(f"[ROTATION] 🌙 WEEK-END MODE: Rotation exclusive Crypto via OTC Provider")
        print(f"[ROTATION] 🎯 Paires Crypto disponibles: {', '.join(otc_provider.get_available_pairs())}")
    else:
    ...
    for batch_start in range(0, len(shuffled_pairs), ROTATION_CONFIG['pairs_per_batch']):
        ...
        for pair in batch_pairs:
            ...
            try:
                ...
                if is_weekend:
                    print(f"[ROTATION] 🌙 {pair} → {actual_pair} ({total_analyzed}ème Crypto via OTC)")
                else:
                    print(f"[ROTATION] 📊 Analyse {pair} ({total_analyzed}ème via TwelveData)")
                ...
                if df is None or len(df) < ROTATION_CONFIG['min_data_points']:
                    result = {
                        ...
                        'data_source': 'OTC' if is_weekend else 'TwelveData'
                    }
                ...
                print(f"[ROTATION] ✅ {actual_pair}: Score {current_score:.1f} (Source: {'OTC' if is_weekend else 'TwelveData'})")
    ...
    if best_signal and best_score >= ROTATION_CONFIG['min_score_threshold']:
        ...
        source = "OTC" if is_weekend else "TwelveData"
        print(f"[ROTATION] ✅ Meilleur signal {pair_type}: {best_signal['pair']} (Score: {best_score:.1f}, Source: {source})")
        return best_signal, total_analyzed, batch_count, analysis_results
    
    pair_type = "Crypto" if is_weekend else "Forex"
    source = "OTC" if is_weekend else "TwelveData"
    print(f"[ROTATION] ❌ Aucun signal valide après {total_analyzed} paires {pair_type} (Source: {source})")
...
async def create_signal_button(user_id: int, app, message_id: int = None) -> int:
    ...
    try:
        if message_id:
            ...
            is_weekend = otc_provider.is_weekend()
            mode_text = "🌙 Mode Crypto (OTC)" if is_weekend else "📈 Mode Forex (TwelveData)"
...
async def generate_m1_signal_with_iterative_rotation(user_id, app):
    """
    Génère un signal avec rotation itérative - INTÉGRATION OTC
    Rotation Crypto optimisée pour week-end avec données OTC
    ...
    try:
        ...
        is_weekend = otc_provider.is_weekend()
        if is_weekend:
            print(f"[SIGNAL] 🌙 MODE WEEK-END: Rotation exclusive sur paires Crypto via OTC Provider")
            print(f"[SIGNAL] 🎯 Sources OTC: Bybit, Binance, KuCoin, CoinGecko, CoinCap, Synthétique")
        
        # Analyse multi-marchés avec OTC Provider
        ...
        if signal_data is None:
            pair_type = "Crypto" if is_weekend else "Forex"
            source = "OTC" if is_weekend else "TwelveData"
            print(f"[SIGNAL] ❌ Aucun signal valide trouvé après analyse {pair_type} (Source: {source})")
        ...
        data_source = signal_data.get('data_source', 'TwelveData')
        
        print(f"[SIGNAL] 🎯 Meilleur signal: {pair} -> {direction} (Score: {score:.1f}, Source: {data_source})")
        ...
        payload = {
            'pair': actual_pair,
            'direction': ml_signal, 
            'reason': f"{reason} | {batch_info} | Source: {data_source}",
            ...
            'payload_json': json.dumps({
                ...
                'rotation_info': {
                    ...
                    'otc_available_pairs': otc_provider.get_available_pairs() if is_weekend_mode else [],
        ...
        print(f"[SIGNAL] ✅ Signal #{signal_id} persisté (Source: {data_source})")
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        is_weekend = otc_provider.is_weekend()
        mode_text = "🏖️ OTC (Crypto)" if is_weekend else "📈 Forex (TwelveData)"
        
        if is_weekend:
            crypto_list = "\n".join([f"• {pair}" for pair in otc_provider.get_available_pairs()])
            mode_text += f"\n🎯 Sources OTC: Bybit, Binance, KuCoin, CoinGecko, CoinCap\n"
            mode_text += f"🔄 Fallback: Données synthétiques"
        ...
        await update.message.reply_text(
            ...
            f"🌙 {len(otc_provider.get_available_pairs())} paires Crypto week-end (OTC)\n"
            ...
            f"• /otcstatus - Statut OTC Provider\n"
        )
    except Exception as e:
...
async def cmd_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_weekend:
        crypto_section = f"🌙 **MODE WEEK-END OTC:**\n• Rotation exclusive Crypto\n• {len(otc_provider.get_available_pairs())} paires via OTC Provider\n• Sources: Bybit, Binance, KuCoin, CoinGecko, CoinCap\n"
    else:
        crypto_section = f"📈 **MODE FOREX:**\n• Rotation standard via TwelveData\n• {len(ROTATION_PAIRS)} paires analysées\n"
...
async def cmd_start_session(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_weekend:
        crypto_details = f"🎯 Sources OTC: Bybit, Binance, KuCoin, CoinGecko, CoinCap\n🔄 Fallback: Données synthétiques"
        mode_text += f"\n{crypto_details}"
...
async def cmd_otc_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Affiche le statut du OTC Provider"""
    is_weekend = otc_provider.is_weekend()
    available_pairs = otc_provider.get_available_pairs()
    
    status_text = (
        f"🌐 **STATUT OTC PROVIDER**\n"
        f"━━━━━━━━━━━━━━━━━━━━\n\n"
        f"📊 Mode week-end: {'✅ ACTIF' if is_weekend else '⏸️ INACTIF'}\n"
        f"🎯 Paires disponibles: {len(available_pairs)}\n\n"
        f"📡 **Sources de données:**\n"
        f"1. Bybit (Spot API) - Priorité haute\n"
        f"2. Binance (Spot API)\n"
        f"3. KuCoin (Spot API)\n"
        f"4. CoinGecko (API publique)\n"
        f"5. CoinCap (API publique)\n"
        f"6. Données synthétiques (fallback)\n\n"
        f"🔄 **Paires supportées:**\n"
    )
    ...
    status_text += f"\n⚡ **Fonctionnement:**\n"
    status_text += f"• En week-end: utilise OTC Provider pour les crypto\n"
    status_text += f"• En semaine: utilise TwelveData pour le Forex\n"
    status_text += f"• Fallback: données synthétiques si toutes les sources échouent\n"
    status_text += f"• Cache: 30 secondes pour données réelles, 5s pour synthétiques"
    
    await update.message.reply_text(status_text)

# ================= AUTRES COMMANDES (garder le reste inchangé) =================
# [Le reste des commandes reste inchangé car elles utilisent déjà otc_provider.is_weekend()]
...
async def callback_generate_signal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Déterminer mode
    is_weekend = otc_provider.is_weekend()
    mode_text = "🌙 Crypto (OTC)" if is_weekend else "📈 Forex (TwelveData)"
    ...
    if signal_id:
        ...
        if analysis_results:
            ...
            analyzed_pairs_text = f"📊 **Paires analysées (Source: {data_source}):**\n"
            ...
            for batch_num in sorted(batches.keys()):
                ...
                for i, result in enumerate(batch_results, 1):
                    ...
                    source = result.get('data_source', data_source)
                    
                    if status == 'SIGNAL_FOUND':
                        score = result.get('score', 0)
                        direction = result.get('direction', 'N/A')
                        direction_emoji = "↗️" if direction == "CALL" else "↘️"
                        analyzed_pairs_text += f"{i}. {pair_display} {direction_emoji} - Score: {score:.1f} ({source})\n"
                    elif status == 'NO_SIGNAL':
                        analyzed_pairs_text += f"{i}. {pair_display} ❌ - Pas de signal ({source})\n"
                    elif status == 'ERROR':
                        reason = result.get('reason', 'Erreur')
                        analyzed_pairs_text += f"{i}. {pair_display} ⚠️ - {reason[:30]} ({source})\n"
                    else:
                        analyzed_pairs_text += f"{i}. {pair_display} ❓ - État inconnu ({source})\n"
            ...
            analyzed_pairs_text += f"• Source: {data_source}\n"
        else:
...
async def cmd_rotation_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_weekend:
        crypto_section = f"🌙 **MODE WEEK-END ACTIF (OTC)**\n• Rotation exclusive sur {len(otc_provider.get_available_pairs())} paires Crypto\n• Sources: Bybit, Binance, KuCoin, CoinGecko, CoinCap\n"
        pairs_text = f"🎯 Paires Crypto: {len(otc_provider.get_available_pairs())}"
    else:
...
async def cmd_button_config(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
        ...
        f"• Voir /otcstatus pour statut OTC\n"
...
async def cmd_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        mode_text = f"🌙 Week-end (Crypto via OTC)" if is_weekend else f"📈 Forex (TwelveData)"
        ...
        msg = (
            ...
            f"📈 Win rate: {winrate:.1f}%\n\n"
            f"🔄 **Rotation:**\n"
            f"• Paires analysées: {len(otc_provider.get_available_pairs()) if is_weekend else len(ROTATION_PAIRS)}\n"
...
async def cmd_cryptolist(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    crypto_pairs = otc_provider.get_available_pairs()
    ...
    msg = (
        ...
        f"Source: OTC Provider (Bybit, Binance, KuCoin, CoinGecko, CoinCap)\n\n"
        f"{crypto_text}\n\n"
        f"🔧 **Fonctionnement:**\n"
        f"• Le week-end (samedi et dimanche UTC)\n"
        f"• Toutes les paires Forex sont transformées en Crypto\n"
        f"• Rotation exclusive via OTC Provider\n"
        f"• Distribution équitable via hash MD5\n"
        f"• Fallback: données synthétiques si échec API\n\n"
        f"🎯 **Sources de données:**\n"
        f"1. Bybit Spot API (priorité haute)\n"
        f"2. Binance Spot API\n"
        f"3. KuCoin Spot API\n"
    ...
    if analysis_results:
        ...
        for i, result in enumerate(recent_results, 1):
            ...
            if status == 'SIGNAL_FOUND':
                ...
                direction = result.get('direction', 'N/A')
                direction_emoji = "↗️" if direction == "CALL" else "↘️"
                msg += f"{i}. {pair} {direction_emoji} ✅ Score: {score:.1f} ({source})\n"
            elif status == 'NO_SIGNAL':
                msg += f"{i}. {pair} ❌ Pas de signal ({source})\n"
            elif status == 'ERROR':
                reason = result.get('reason', 'Erreur')
                msg += f"{i}. {pair} ⚠️ {reason[:30]} ({source})\n"
            else:
                msg += f"{i}. {pair} ❓ Inconnu ({source})\n"
...
async def health_check(request):
    ...
    return web.json_response({
        ...
        'crypto_pairs': len(otc_provider.get_available_pairs()),
        'weekend_mode': is_weekend,
        'data_source': 'OTC Provider' if is_weekend else 'TwelveData',
        'button_timeout': BUTTON_TIMEOUT_MINUTES,
        'button_after_candle': 'active',
        'analysis_display': 'active',
        'otc_integration': 'active',
        'fallback_synthetic': 'available'
    })
...
async def main():
    ...
    print("🌙 ROTATION CRYPTO OPTIMISÉE WEEK-END (OTC PROVIDER)")
    print("📊 AFFICHAGE DÉTAILLÉ DES PAIRES ANALYSÉES")
    print("🔧 VERSION CORRIGÉE - INTÉGRATION OTC COMPLÈTE")
    print("="*60)
    print(f"🎯 Stratégie: Saint Graal 4.5 avec Rotation Itérative")
    print(f"📊 Paires Forex analysées: {len(ROTATION_PAIRS)}")
    print(f"🌙 Paires Crypto week-end: {len(otc_provider.get_available_pairs())}")
    print(f"📡 Sources OTC: Bybit, Binance, KuCoin, CoinGecko, CoinCap")
    print(f"🔄 Fallback: Données synthétiques")
    ...
    print(f"🔧 Intégration OTC: ✅ COMPLÈTE")
    ...
    is_weekend = otc_provider.is_weekend()
    if is_weekend:
        print(f"🌙 MODE WEEK-END ACTIF: Rotation exclusive Crypto via OTC Provider")
        print(f"🎯 Paires Crypto disponibles: {', '.join(otc_provider.get_available_pairs())}")
        print(f"📡 Sources: Bybit, Binance, KuCoin, CoinGecko, CoinCap")
        print(f"🔄 Fallback: Données synthétiques")
    else:
        print(f"📈 MODE FOREX ACTIF: Rotation standard via TwelveData")
        print(f"📊 Paires Forex: {len(ROTATION_PAIRS)}")
    
    print(f"🔄 Bouton après bougie: ✅ ACTIVÉ")
    print(f"📊 Affichage analyses: ✅ ACTIVÉ")
    print(f"🔧 Intégration OTC Provider: ✅ COMPLÈTE")
    print(f"⏱️ Bouton timeout: {BUTTON_TIMEOUT_MINUTES} min")
    print(f"🔧 Utilisez /lastanalysis pour voir les paires analysées")
    print(f"🔧 Utilisez /otcstatus pour vérifier le statut OTC Provider")
